...
# from condface_detector import YoloV5FaceDetector
    ...
    while True:
        ...
        if grabbed != True:
            if start_idx is not None and end_idx == None:
                end_idx = (frames/fps)*1000 #cur_frame_idx
        ...
        if cur_frame_idx % frames_per_detect == 0:
            ...
            if len(rec_dist)>0:
                ...
                elif max(rec_dist) < dist_thresh and start_idx != None:
                    end_idx = cap.get(cv2.CAP_PROP_POS_MSEC) #cur_frame_idx #- frames_per_detect
                    time.append((start_idx, end_idx))
                    start_idx = None
                    end_idx = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--dist_thresh", type=float, default=0.5, help="Cosine dist thresh, dist lower than this will be Unknown")
    parser.add_argument("-p", "--frames_per_detect", type=int, default=5, help="Do detect every [NUM] frame")
    parser.add_argument("-o", "--output", type=str, help="output folder")
    # parser.add_argument("-s2t", "--speech_to_text", type=bool, default=True, help="Run speech to text")
    ...
    if known_user != None and face_model is not None:
        ...
        timestamps = video_recognize(image_classes, embeddings, det, face_model, video_source, args.frames_per_detect, args.dist_thresh)
    end_fr = time.time()
    ...
    with open(f"{output_folder}/stage_output_and_runtime.txt", "a+", encoding='utf-8') as sr:
        # f.write(str((start_time, end_time)) + "\n")
        # sr.write(f"{video_name}:\n")
        sr.write(f"1st stage output: {timestamps}\n")
        sr.write(f"1st stage runtime: {end_fr-start}\n")
        sr.write(f"2nd stage output:\n")

    # if args.speech_to_text:
        start_sound = time.time()
        clip = moviepy.editor.VideoFileClip(args.video_source)
        audio_file = f"{output_folder}/video_sound.wav"
        clip.audio.write_audiofile(audio_file)
        # open(f"{args.output}/video_output.txt", 'w+').close()
        ...
        for idx, interval in enumerate(timestamps):        
            ...
            # # write output 1st stage
            # with open(f"{args.output}/output_and_time.txt", "a+", encoding='utf-8') as f:
            #     # f.write(str((start_time, end_time)) + "\n")
            #     f.write(str((interval[0], interval[1])) + "\n")

            output = f"{output_folder}/audio_{idx}.wav"

            # extract audio from file
            newAudio = AudioSegment.from_wav(audio_file)
            # newAudio = newAudio[int(start_time*1000):int(end_time*1000)]
            newAudio = newAudio[interval[0]:interval[1]]
            ...
            sr.write(f"\t---Timestamp {idx}: {(interval[0], interval[1])}--- \n")
            sr.write(f"\t{text}\n")
        end_sound = time.time()
        # print(f"time for face recognize: {end_fr - start}\ntime for sound extraction: {end_sound - end_fr}")
        
        sr.write(f"2d stage runtime: {end_sound-start_sound}\n")
        sr.write(f"total runtime: {end_sound-start}") 
